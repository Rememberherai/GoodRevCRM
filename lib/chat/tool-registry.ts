...
defineTool({
  ...
  parameters: z.object({
    ...
    new_household_name: z.string().max(200).optional().describe('Create a new household with this name and link the person'),
    custom_fields: z.record(z.string(), z.unknown()).optional().describe('Custom field values'),
    disposition_id: z.string().uuid().nullable().optional().describe('Disposition ID (status category)'),
  }),
  handler: async (params, ctx) => {
    const { organization_id, household_id, household_relationship, household_is_primary_contact, new_household_name, custom_fields, ...personFields } = params as Record<string, unknown>;
    ...
    if (household_id) {
      ...
      const { error: memberErr } = await ctx.supabase
        ...
        .insert({
          ...
          person_id: data.id,
          relationship: ((household_relationship as string) || 'other'),
      ...
      if (hhErr) {
        ...
        await ctx.supabase
          ...
          .insert({
            ...
            person_id: data.id,
            relationship: ((household_relationship as string) || 'head_of_household'),
            is_primary_contact: household_is_primary_contact === true,
            start_date: new Date().toISOString().split('T')[0],
          } as any);
...
defineTool({
  name: 'documents.list',
  description: 'List standalone and project documents with pagination and filtering',
  minRole: 'viewer',
  parameters: z.object({
    page: z.number().int().min(1).default(1).describe('Page number'),
    limit: z.number().int().min(1).max(100).default(50).describe('Items per page'),
    search: z.string().optional().describe('Search by title'),
    status: z.string().optional().describe('Filter by status'),
    standalone_only: z.boolean().optional().describe('Only standalone documents'),
  }),
  handler: async (params, ctx) => {
    const { page = 1, limit = 50, search, status, standalone_only } = params as {
      page?: number; limit?: number; search?: string; status?: string; standalone_only?: boolean;
    };
    const offset = (page - 1) * limit;
    let query = ctx.supabase
      .from('contract_documents')
    ...
    const { data, error, count } = await query;
    if (error) throw new Error(`Failed: ${error.message}`);
    return JSON.stringify({ documents: data, pagination: { page, limit, total: count ?? 0 } });
  },
});

defineTool({
  name: 'documents.get',
  description: 'Get a standalone document by ID with recipients',
  minRole: 'viewer',
  parameters: z.object({
...
defineTool({
  ...
  description: 'Create a new standalone document from an uploaded PDF path',
  minRole: 'member',
  parameters: z.object({
    title: z.string().min(1).describe('Document title'),
...
defineTool({
  name: 'documents.void',
  description: 'Void an active standalone document',
  minRole: 'member',
  parameters: z.object({
    id: z.string().uuid().describe('Document ID'),
    reason: z.string().optional().describe('Reason for voiding'),
  ...
  handler: async (params, ctx) => {
    ...
    if (nonVoidable.includes(doc.status)) throw new Error(`Cannot void a ${doc.status} document`);
...
defineTool({
  name: 'documents.audit_trail',
  description: 'View the audit trail for a standalone document',
  minRole: 'viewer',
  parameters: z.object({
  ...
  handler: async (params, ctx) => {
    ...
    const { data: doc, error: docError } = await ctx.supabase
      ...
      .is('deleted_at', null)
      .single();
    if (docError || !doc || !canAccessDocument(ctx, doc)) throw new Error('Document not found');
...
defineTool({
  ...
  description: 'List document templates available for standalone documents',
  minRole: 'viewer',
  parameters: z.object({}),
  handler: async (_params, ctx) => {
    const { data, error } = await ctx.supabase
...
defineTool({
  ...
  parameters: z.object({
    ...
    notes: z.string().optional().describe('Internal payment notes'),
  }),
  handler: async (params, ctx) => {
...
defineTool({
  ...
  description: 'List bug reports with optional status filtering and pagination (admin only)',
  minRole: 'admin',
  parameters: z.object({
  ...
  handler: async (params, ctx) => {
    const { status, page = 1, limit = 50 } = params as {
...
defineTool({
  name: 'bug_reports.update_status',
  description: 'Update the status of a bug report (admin only)',
  minRole: 'admin',
  parameters: z.object({
  ...
  handler: async (params, ctx) => {
    const { id, status, resolution_notes } = params as {
      id: string; status: string; resolution_notes?: string;
    };
