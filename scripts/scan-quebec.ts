...
async function scanQuebec() {
  ...
  logger.logHeader();
  console.log('⚜️  Quebec Municipal RFP Scanner');
  console.log('====================================\n');

  // Fetch all Quebec municipalities
  ...
  if (!municipalities || municipalities.length === 0) {
    console.log('⚠️  No Quebec municipalities found with minutes URLs');
    console.log('\n💡 Tip: Run the CSV import first:');
    console.log('   npm run add-quebec-from-csv\n');
    return;
  }

  console.log(`Found ${municipalities.length} Quebec municipalities with minutes URLs\n`);
  ...
  for (let i = 0; i < municipalities.length; i++) {
    ...
    console.log(`[${i + 1}/${municipalities.length}] ${municipality.name}, Quebec`);
    console.log(`  📄 Minutes URL: ${municipality.minutes_url || 'N/A'}`);
    console.log(`  👥 Population: ${municipality.population?.toLocaleString() || 'Unknown'}`);

    if (!municipality.minutes_url) {
      console.log(`  ⚠️  No minutes URL - skipping\n`);
      continue;
    }
  ...
  logger.logSummary({
    ...
    provinceBreakdown: { Quebec: totalRfps },
  });
