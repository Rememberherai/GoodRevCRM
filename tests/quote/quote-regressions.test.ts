...
describe('Quote Regression Guards', () => {
  it('keeps bulk line item replacement atomic in the DB layer', () => {
    const migration = readProjectFile('supabase', 'migrations', '0097_quotes_and_line_items.sql');
  ...
  it('keeps oppId scoping on nested line-item routes', () => {
    ...
    expect(route).toContain('const { slug, oppId, quoteId, itemId } = await context.params;');
    expect(route).toContain('quoteId, itemId, body, oppId');
    expect(route).toContain('quoteId, itemId, oppId');
  });
