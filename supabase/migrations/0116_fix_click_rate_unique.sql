-- Fix click_rate to use unique clicks (per sent email) instead of total clicks.
-- Same pattern as open_rate which already uses unique_opens.
-- Also add unique_bounces for consistency across all rate calculations.
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'click_rate', CASE WHEN v_total_sent > 0
      THEN ROUND(COALESCE(v_unique_clicks, 0)::NUMERIC / v_total_sent * 100, 1)
      ELSE 0
    END,
