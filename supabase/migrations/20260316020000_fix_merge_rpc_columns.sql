-- Fix: Corrects perform_merge RPC to use correct column names (merged_by, merged_records_snapshot)
-- and correct return structure (merge_history_id). The previous migration (20260316010000) introduced
-- column name mismatches and parameter defaults. Must DROP and recreate to remove defaults.

DROP FUNCTION IF EXISTS perform_merge(UUID, TEXT, UUID, UUID[], JSONB, UUID);
