...
import { CalendarDays, ChevronLeft, ChevronRight, MapPin, Monitor, Clock, Play, Rss, Copy, Check } from 'lucide-react';
        ...
        <CardContent className="p-0">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[48px] sm:min-h-[80px] border-r border-b last:border-r-0 p-1 sm:p-1.5 transition-colors ${
                    dayEvents.length > 0 ? 'cursor-pointer hover:bg-primary/5' : ''
                  } ${isToday ? 'bg-primary/5 ring-inset ring-2 ring-primary/30' : ''
                  } ${isSelected ? 'bg-primary/10 ring-inset ring-2 ring-primary/50' : ''
                  } ${isPastDay && !isToday && !isSelected ? 'bg-muted/5' : ''}`}
                >
                  {/* Date number */}
          ...
          {selectedDate && (
            <div className="px-4 py-2 border-t bg-muted/20">
              ...
              >
                Showing events for {selectedDate.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })} — click to show all
              </Button>
            </div>
          )}
        </CardContent>
